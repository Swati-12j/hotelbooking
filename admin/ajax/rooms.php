    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $data .= "
            <tr class='align-middle'>
                ...
                <button  onclick=\"room_images($row[id],'$row[name]')\"  type='button' class='btn btn-info shadow-none btn-sm' data-bs-toggle='modal'
                data-bs-target='#room-images'>
                <i class='bi bi-images'></i>
                </button>

                <button  onclick='remove_room($row[id])'  type='button' class='btn btn-danger shadow-none btn-sm'>
...
if(isset($_POST['get_room'])){
   $frm_data =filteration($_POST);
    ...
    $res1=select("SELECT * FROM `rooms_features` WHERE `room_id`=?",[$FRM_DATA['get_room']], 'i');
    $res1=select("SELECT * FROM `rooms_facilities` WHERE `room_id`=?",[$FRM_DATA['get_room']], 'i');
...

{
  ...
  $values = [$frm_data['name'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc'],$frm_data['id']];
  if(update($q1,$values,'siiiiisi')){
   $flag=1;

  }
  $del_features =delete("DELETE FROM `room_feactures` WHERE `room_id`=?",[$frm_data['room_id']],'i');
...
if (isset($_POST['remove_room'])) {
    ...
    while($row =mysqli_fetech_assoc($res1)){
